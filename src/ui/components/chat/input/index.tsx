...
import {getProviderModels, PROVIDERS, DEFAULT_CLAUDE_MODEL, DEFAULT_PROVIDER, DEFAULT_QUICK_CHAT_MODEL} from "../../../data/models";
...
const ChatInput = ({
    ...
    const [localImages, setLocalImages] = useState<{id: string; src: string; file: File}[]>([]);
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
        if (showAutocomplete) {
            if (e.key === "ArrowDown") {
                e.preventDefault();
                setHighlightedIndex((prev) => (prev < autocompleteItems.length - 1 ? prev + 1 : 0));
            } else if (e.key === "ArrowUp") {
                e.preventDefault();
                setHighlightedIndex((prev) => (prev > 0 ? prev - 1 : autocompleteItems.length - 1));
    ...
    const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        ...
        if (currentWord.startsWith("/")) {
            ...
            const filtered = snippets
                .filter((snip) => snip.trigger.toLowerCase().includes(currentWord.toLowerCase()))
                .map((snip) => ({
                    id: snip.id,
    ...
    return (
        <div className="absolute bottom-4 left-0 right-0 px-6 py-4">
                ...
                <div className="relative">
                    ...
                    {showAutocomplete && (
                        <AutocompletePalette
                            ...
                            onManageSnippets={triggerChar === "@" ? handleManageSnippets : undefined}
                        />
                    )}
                    ...
                    <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex items-center gap-3">
                            {mode === "claude-code" ? (
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    feedbackEnabled ? "bg-accent-primary" : "bg-bg-tertiary",
                                                    disabled && "opacity-50 cursor-not-allowed"
                                                )}
                                            >
                                                <span
                                                    className={cn(
                                                        ...
                                                        feedbackEnabled ? "translate-x-4 bg-text-inverse" : "translate-x-0 bg-white"
                                                    )}
                                                />
                                            </button>
                                            <span className="text-sm text-text-secondary">{t("chat.feedback")}</span>
                                ...
                                <>
                                    ...
                                    {/* 해당 제공자의 모델 선택 */}
                                    <ModelSelect
                                        models={providerModels[selectedProvider || DEFAULT_PROVIDER] || []}
                                        selectedModelId={selectedModel || DEFAULT_QUICK_CHAT_MODEL}
                                        onModelChange={onModelChange}
                                        disabled={disabled}
                                    />
                                </>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {mode === "claude-code" && showSummaryButton && (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        showSummaryButton && "animate-pulse text-warning hover:text-warning",
                                        (disabled || summaryMutation.isPending) && "opacity-50 cursor-not-allowed"
                                    )}
                                    title={summaryMutation.isPending ? t("chat.summarizing") : t("chat.contextSummary")}
                            ...
                            <button
                                onClick={handleSend}
                                disabled={(!content.trim() && allImages.length === 0) || disabled || feedbackMutation.isPending}
                                className={cn(
                                    "p-2 rounded-md transition-all",
                                    (content.trim() || allImages.length > 0) && !disabled && !feedbackMutation.isPending
                                        ? "bg-accent-primary text-text-inverse hover:bg-accent-hover"
                                        : "bg-bg-tertiary text-text-secondary cursor-not-allowed"
                                )}
                                title={feedbackMutation.isPending ? t("chat.requestingFeedback") : t("chat.send")}
                            >
                                <ArrowUp size={18} />
                            </button>
                        </div>
