...
const QuestionCard = ({sessionId, clientId, questionData, isSubmitted = false, questionAnswers}: QuestionCardProps) => {
    ...
    // 제출 핸들러
    const handleSubmit = () => {
        if (!isValid || isSubmitting || isSubmitted || !clientId) return;
        ...
        submitAnswer(sessionId, clientId, questionData.tool_use_id, answers);
    };
        ...
        <div className="question-card bg-accent-primary/5 border border-accent-primary/20 rounded-lg p-4 my-2 space-y-4">
                ...
                return (
                    <div key={qIdx} className="space-y-2">
                        ...
                        <div className="flex items-center gap-2">
                            <span className="inline-block px-2 py-0.5 text-xs font-medium bg-accent-primary/10 text-accent-primary rounded">{q.header}</span>
                            {!isSubmitted && q.multiSelect && <span className="text-xs text-text-secondary">{t("question.multiSelect")}</span>}
                        </div>

                        {/* 질문 텍스트 */}
                        <p className={`text-sm font-medium ${isSubmitted ? "text-text-secondary" : "text-text-primary"}`}>{q.question}</p>

                        {/* 선택지 목록 */}
                        <div className="space-y-2 ml-2">
                            {q.options.map((option, oIdx) => {
                                const selected = isSubmitted ? submittedLabels.includes(option.label) : isSelected(qIdx, oIdx);
                                return (
                                    <button
                                        key={oIdx}
                                    ...
                                    .map((otherLabel, idx) => (
                                        ...
                                        >
                                            <div className="flex items-start gap-3">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="text-sm font-medium text-accent-primary">{otherLabel}</div>
                                                    <div className="text-xs mt-1 text-text-tertiary">{t("question.other")}</div>
                                                </div>
                                            </div>
                                        </div>
                                    ))
            ...
            {!isSubmitted && !isCliSession && (
                <div className="pt-2 border-t border-border-default">
                    ...
                    {!isValid && <p className="text-xs text-text-tertiary text-center mt-2">{t("question.allRequired")}</p>}
                </div>
            )}
